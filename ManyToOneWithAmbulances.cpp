...
public:
    ...
    int drivingTime;
    bool emergency;
    Car(int s = -1, int c = -1, bool e = false): arrivalTime(s), drivingTime(c), emergency(e){}
        ...
        for(int j = 0; j < indicesBySize[i].size(); j++){
            ...
            for(int lane = 0; lane < carCnt.size(); lane++){
                ...
                for(int fromLane = 0; fromLane < carCnt.size(); fromLane++){
                    ...
                    if(table[fromLane][curIdx - g.singleLane(fromLane)].completionTime == INT32_MAX)    continue;
                ...
                if(optTime == INT32_MAX || (cars[lane][indicesBySize[i][j][lane]].emergency && optTime > cars[lane][indicesBySize[i][j][lane]].arrivalTime)){
                    table[lane][curIdx].change(-1, INT32_MAX);
                    continue;
                }
    ...
    for(int lane = 0; lane < N; lane++){
        ...
        if (carCnt[lane] > 0){
            for(int i = 0; i < carCnt[lane]; i++){
                cin >> s >> c >> e; cars.back().emplace_back(Car(s,c,e==1));
            }
        }
    }
